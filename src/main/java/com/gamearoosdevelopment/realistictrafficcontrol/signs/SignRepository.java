...
public class SignRepository {
	...
	public void init(Consumer<String> splashUpdate, IntConsumer maximumUpdate)
	{
		...
		for(File file : signpackDir.listFiles())
		{
			...
			try
			{
				...
				while(enumeration.hasMoreElements())
				{
					...
					if (entry.getName().toLowerCase().equals("signs.json"))
					{
						...
						{
							ModRealisticTrafficControl.logger.error("Could not read or parse signs.json in signpack " + file.getName() , ex);
							continue;
						}
					}
				}
			...
			{
				ModRealisticTrafficControl.logger.error("Could not process signpack " + file.getName() + "." , ex);
				continue;
			}
			finally
			{
	...
	private void processSignFile(JsonObject signsFile, ZipFile zipFile, Consumer<String> splashUpdate, IntConsumer stepsUpdate) throws Exception
	{
		...
		{
			ModRealisticTrafficControl.logger.error("Could not parse pack_id string to a uuid. This pack will not be loaded.", ex);
			return;
		}
		...
		{
			ModRealisticTrafficControl.logger.error("Could not retrieve name for pack id " + packID.toString() + ". This pack will not be loaded.", ex);
			return;
		}
		
		if (packNamesByID.containsKey(packID))
		{
			ModRealisticTrafficControl.logger.error("Pack " + name + " has already been loaded! Do you have a duplicate pack installed?");
			return;
		}
	...
	private Sign getSignFromObject(JsonObject signObject, UUID packID, Consumer<String> splashUpdate) throws Exception
	{
		...
		ResourceLocation frontRL = new ResourceLocation("realistictrafficcontrol", "textures/blocks/signs/" + packID.toString() + "/" + type + "/" + frontTextureName);
		...
		ResourceLocation backRL = new ResourceLocation("realistictrafficcontrol", "textures/blocks/signs/" + packID.toString() + "/" + type + "/" + backTextureName);
	...
	{
		ResourceLocation signJsonRL = new ResourceLocation("realistictrafficcontrol:misc/signs.json");
	...
	{
		return ModRealisticTrafficControl.class.getClassLoader().getResourceAsStream("assets/realistictrafficcontrol/misc/signs.json");
	}
	...
	private void loadSignTexture(Sign sign, ZipFile zipFile, HashSet<UUID> signsToRemove) throws Exception
	{
		...
		if (Minecraft.getMinecraft().getTextureManager().getTexture(frontTexture) == null)
		{
			...
			if (zipFile != null) // load from zip file
			{
				...
				if (entry == null)
				{
					ModRealisticTrafficControl.logger.error("Could not find image '" + entryPath + "' for sign '" + sign.getID().toString() + "'. It will not be available.");

...
//        GlStateManager.rotate(mc.player.rotationPitch, 1, 0, 0);
		...
		addModelToBuffer(facings, manager, builder, "realistictrafficcontrol:" + modelPrefix, baseModelVariant);
		if (te.getNwBulbRotation() >= 0)
		{
			addModelToBuffer(facings, manager, builder, "realistictrafficcontrol:" + modelPrefix + "_nw_lamp", "rotation=" + te.getNwBulbRotation() + ",state=off");
		}

		if (te.getNeBulbRotation() >= 0)
		{
			addModelToBuffer(facings, manager, builder, "realistictrafficcontrol:" + modelPrefix + "_ne_lamp", "rotation=" + te.getNeBulbRotation() + ",state=off");
		}

		if (te.getSwBulbRotation() >= 0)
		{
			addModelToBuffer(facings, manager, builder, "realistictrafficcontrol:" + modelPrefix + "_sw_lamp", "rotation=" + te.getSwBulbRotation() + ",state=off");
		}

		if (te.getSeBulbRotation() >= 0)
		{
			addModelToBuffer(facings, manager, builder, "realistictrafficcontrol:" + modelPrefix + "_se_lamp", "rotation=" + te.getSeBulbRotation() + ",state=off");
		}

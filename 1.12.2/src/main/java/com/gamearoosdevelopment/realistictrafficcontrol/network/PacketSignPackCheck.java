...
public class PacketSignPackCheck implements IMessage {
	...
	public static class Handler implements IMessageHandler<PacketSignPackCheck, IMessage>
	{
		...
		{			
			ImmutableMap<UUID, String> packs = ModRealisticTrafficControl.instance.signRepo.getPacksByID();
			for(Entry<UUID, String> serverPack : message.signPacks.entrySet())
			{
				if (!packs.containsKey(serverPack.getKey()))
				{
					Minecraft.getMinecraft().player.sendMessage(new TextComponentString("You are missing Realistic Traffic Control signpack " + serverPack.getValue() + "! Some signs may display as ERROR."));
				}
			}
		}
	}

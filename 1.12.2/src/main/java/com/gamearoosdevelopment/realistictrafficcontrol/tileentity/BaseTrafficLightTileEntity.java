...
public class BaseTrafficLightTileEntity extends TileEntity implements ITickable {
	...
	public void setConfiguredApproachFacing(EnumFacing facing) {
		...
		if (world != null && !world.isRemote) {
			TrafficLightControlBoxTileEntity.notifyApproachFacingChanged(world, getPos());
		}
	}
	...
	public void setActive(EnumTrafficLightBulbTypes bulbType, boolean active, boolean flash) {
		...
		// Clear solid-red when green or yellow balls light — red-arrow aliases must not leave
		// red+green or red+yellow on the same head.
	...
	void applySolidRedArrowAliases(EnumTrafficLightBulbTypes bulbType, boolean active, boolean flash) {
		// Don't alias to solid-red when the signal is already displaying solid green/yellow.
	...
	private boolean isRightRedArrow(EnumTrafficLightBulbTypes bulbType) {
		return bulbType == EnumTrafficLightBulbTypes.RedArrowRight || bulbType == EnumTrafficLightBulbTypes.RedArrowRight2;
	}

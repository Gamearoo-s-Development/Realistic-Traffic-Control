...
import com.gamearoosdevelopment.realistictrafficcontrol.blocks.BlockRelayBase;
...
import com.gamearoosdevelopment.realistictrafficcontrol.tileentity.CrossingGateGateTileEntity.EnumStatuses;
...
public class RelayTileEntity extends SyncableTileEntity implements ITickable, IScannerSubscriber, IHasRotationProperty { // implements IHasRotationProperty to *prevent* rotation
	...
	private void notifyLamps()
	{
		...
		for(BlockPos positionToRemove : positionsToRemove)
		{
			...
			ModRealisticTrafficControl.logger.error("Crossing Lamp at " + positionToRemove.getX() + ", " + positionToRemove.getY() + ", " + positionToRemove.getZ() + " has been unpaired due to an error");
		}
	}
	...
	private boolean notifyWigWags()
	{
		if (!alreadyNotifiedWigWags)
		{
			...
			for(BlockPos pos : positionsToRemove)
			{
				...
				ModRealisticTrafficControl.logger.error("Wig Wag at " + pos.getX() + ", " + pos.getY() + ", " + pos.getZ() + " has been unpaired due to an error");
			}
	...
	private boolean notifyVerticalWigWags()
	{
		if (!alreadyNotifiedVerticalWigWags)
		{
			...
			for(BlockPos pos : positionsToRemove)
			{
				...
				ModRealisticTrafficControl.logger.error("Vertical Wig Wag at " + pos.getX() + ", " + pos.getY() + ", " + pos.getZ() + " has been unpaired due to an error");
			}
	...
	private boolean checkRemoveInvalidItems()
	{
		...
		{
			didRemove = true;
			ModRealisticTrafficControl.logger.warn("Crossing Relay at " + getPos().toString() + " found that a crossing gate at " + invalidGate.toString() + " did not load after 10 seconds.  It is getting removed from the list of paired items for this relay.");
		...
		{
			didRemove = true;
			ModRealisticTrafficControl.logger.warn("Crossing Relay at " + getPos().toString() + " found that a bell at " + invalidBell.toString() + " did not load after 10 seconds.  It is getting removed from the list of paired items for this relay.");
	...
	/** Same walk as {@link com.gamearoosdevelopment.realistictrafficcontrol.item.ItemCrossingRelayBox#placeMultiblock}. */

...
public class CrossingLampsRenderer extends TileEntitySpecialRenderer<CrossingLampsTileEntity> {
	...
	public void render(CrossingLampsTileEntity te, double x, double y, double z, float partialTicks,
		...
		else if (blockState.getBlock() instanceof BlockRotatableCrossingLamps)
		{
		...
		{
			addModelToBuffer(facings, manager, builder, "realistictrafficcontrol:" + modelPrefix + "_ne_lamp", "rotation=" + te.getNeBulbRotation() + ",state=" + te.getState().getName());
		}
		if (te.getNwBulbRotation() >= 0)
		{
			addModelToBuffer(facings, manager, builder, "realistictrafficcontrol:" + modelPrefix + "_nw_lamp", "rotation=" + te.getNwBulbRotation() + ",state=" + te.getState().getName());
		}
		if (te.getSeBulbRotation() >= 0)
		{
			addModelToBuffer(facings, manager, builder, "realistictrafficcontrol:" + modelPrefix + "_se_lamp", "rotation=" + te.getSeBulbRotation() + ",state=" + te.getState().getName());
		}
		if (te.getSwBulbRotation() >= 0)
		{
			addModelToBuffer(facings, manager, builder, "realistictrafficcontrol:" + modelPrefix + "_sw_lamp", "rotation=" + te.getSwBulbRotation() + ",state=" + te.getState().getName());
		}
		if (renderPoleExtensions && down)
		{
			addModelToBuffer(facings, manager, builder, "realistictrafficcontrol:crossing_gate_pole_ext", "rotation=down");
		}
		...
		if (renderPoleExtensions)
		{
			...
			if (north)
			{
				addModelToBuffer(facings, manager, builder, "realistictrafficcontrol:crossing_gate_pole_ext", "rotation=north");
			}
			if (west)
			{
				addModelToBuffer(facings, manager, builder, "realistictrafficcontrol:crossing_gate_pole_ext", "rotation=west");
			}
			if (south)
			{
				addModelToBuffer(facings, manager, builder, "realistictrafficcontrol:crossing_gate_pole_ext", "rotation=south");
			}
			if (east)
			{
				addModelToBuffer(facings, manager, builder, "realistictrafficcontrol:crossing_gate_pole_ext", "rotation=east");
			}
			tess.draw();
			GlStateManager.popMatrix();
		}

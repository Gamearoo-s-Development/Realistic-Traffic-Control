...
import com.gamearoosdevelopment.realistictrafficcontrol.util.UnlistedPropertyBoolean;
 ...
 * Used by railroad crossing gate lamps and pedestrian hybrid beacon lamps.
 */
public abstract class BlockRotatableCrossingLamps extends BlockLampBase implements IHorizontalPoleConnectable {
	...
	private boolean checkDirection(IBlockAccess world, BlockPos pos, EnumFacing facing) {
		...
		if (facing == EnumFacing.DOWN && otherState.getBlock().getRegistryName().getResourceDomain().equalsIgnoreCase(ModRealisticTrafficControl.MODID)) {
			return true;
		}
